...
const PedagogicalObservations: React.FC<PedagogicalObservationsProps> = ({ studentId }) => {
  ...
  const studentObservations = mockObservations.filter(
    observation => observation.studentId === studentId
  );
  
  // Apply filters
  const filteredObservations = studentObservations.filter(obs => 
    (typeFilter === 'all' || obs.type === typeFilter) &&
    (priorityFilter === 'all' || obs.priority === priorityFilter)
  );
  ...
  const handleAddObservation = () => {
    toast({
      ...
      description: "O registro de observações estará disponível em breve.",
      variant: "default",
    });
  };
            ...
            filteredObservations.map(observation => (
              ...
              >
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        ...
                        <Badge className={getPriorityColor(observation.priority)}>
                          Prioridade: {getPriorityLabel(observation.priority)}
                        </Badge>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-4 pt-2 border-t border-gray-100 text-sm text-gray-500 flex justify-end">
                  Registrado por: Instrutor ID {observation.instructorId}
                </div>
              </div>
            ))
          )}
