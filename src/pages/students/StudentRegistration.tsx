...
const StudentRegistration: React.FC = () => {
  ...
  const handleConfirmSubmit = () => {
    ...
    const newStudent: Student = {
      ...
      userId: uuidv4(), // This would normally be linked to a user account
      fullName: formData.fullName,
      registrationNumber: generateRegistrationNumber(),
      birthDate: format(formData.birthDate, 'yyyy-MM-dd'),
      email: formData.email || `${formData.fullName.toLowerCase().replace(/\s+/g, '.')}@estudante.cbmepi.gov.br`,
    ...
    // In a real app, we would save the student to a database
    ...
    toast({
      ...
      description: `O aluno ${newStudent.fullName} foi cadastrado com sucesso.`,
      variant: "default",
    });
  ...
  const handleDocumentUpload = (newDocuments: StudentDocument[]) => {
                      ...
                      <FormField
                        ...
                        render={({ field }) => {
                          ...
                          const availableClasses = mockClasses.filter(cls => 
                            selectedCourseIds.includes(cls.courseId)
                          );
