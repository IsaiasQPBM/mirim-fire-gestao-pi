...
const StudentDashboard = () => {
  ...
  const performanceData = mockStudentPerformance
    .filter(perf => perf.studentId === id)
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    .map(perf => ({
      date: new Date(perf.date).toLocaleDateString('pt-BR'),
      performance: (perf.score / perf.maxScore) * 100,
      average: 75 // Mock class average for demonstration
    }));
  ...
  const studentObservations = mockObservations.filter(obs => obs.studentId === id);
  ...
  const calculateOverallPerformance = () => {
    const performances = mockStudentPerformance.filter(perf => perf.studentId === id);
  ...
  const lowPerformanceAreas = mockStudentPerformance
    .filter(perf => perf.studentId === id && (perf.score / perf.maxScore) < 0.7)
    .map(perf => {
      const assessment = mockAssessments.find(a => a.id === perf.assessmentId);
          ...
          <Tabs defaultValue="performance" className="w-full">
                      ...
                      {studentObservations.map((observation) => (
                        <div key={observation.id} className="border rounded-md p-4">
                          ...
                          <div className="text-xs text-gray-500">
                            Registrado por: {mockUsers.find(u => u.id === observation.instructorId)?.fullName}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <TabsContent value="assessments">
              <Card>
                ...
                <CardContent>
                  {mockStudentPerformance.filter(perf => perf.studentId === id).length > 0 ? (
                    <div className="space-y-4">
                      {mockStudentPerformance
                        .filter(perf => perf.studentId === id)
                        .map((performance) => {
                          const assessment = mockAssessments.find(a => a.id === performance.assessmentId);
                          return (
                            <div key={performance.id} className="border rounded-md p-4">
                              <div className="flex flex-wrap items-center justify-between mb-3">
                                <h4 className="text-lg font-medium">{assessment?.title || 'Avaliação'}</h4>
                                <div className="flex items-center space-x-2">
                                  <div className="text-sm text-gray-500">
                                    {new Date(performance.date).toLocaleDateString('pt-BR')}
                                  </div>
                                  <Badge className={`${performance.score >= (performance.maxScore * 0.7) ? 'bg-green-500' : 'bg-red-500'} text-white`}>
                                    {performance.score}/{performance.maxScore} pontos
                                  </Badge>
                                </div>
                              </div>
                              <div className="mb-3">
                                <div className="h-2 w-full bg-gray-200 rounded-full">
                                  <div 
                                    className={`h-2 rounded-full ${performance.score >= (performance.maxScore * 0.7) ? 'bg-green-500' : 'bg-red-500'}`}
                                    style={{ width: `${(performance.score / performance.maxScore) * 100}%` }}
                                  ></div>
                                </div>
