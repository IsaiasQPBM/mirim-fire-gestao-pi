...
import { mockCourses, mockDisciplines } from '@/data/mockCurriculumData';
...
const CurriculumView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Build curriculum data from courses and disciplines
    const curriculumData: CurriculumItem[] = mockCourses.map(course => {
      // Get disciplines for this course
      const courseDisciplines = mockDisciplines.filter(d => d.courseId === course.id);
      
      // Format disciplines data
      const formattedDisciplines = courseDisciplines.map(discipline => ({
        id: discipline.id,
        name: discipline.name,
        theoryHours: discipline.theoryHours,
        practiceHours: discipline.practiceHours,
        totalHours: discipline.theoryHours + discipline.practiceHours,
        isActive: discipline.isActive
      }));
      
      // Calculate total hours from disciplines
      const calculatedTotalHours = formattedDisciplines.reduce((total,  discipline) => {
        return total + discipline.totalHours;
      }, 0);
      
      return {
        courseId: course.id,
        courseName: course.name,
        totalHours: calculatedTotalHours || course.totalHours, // Use calculated hours or fall back to course hours
